...
> 抽象化、local-agent-gateway 経由の LLM レビュー、履歴の JSONL 一本化、
> `evaluate --history` による評価実行の時系列差分は実装済み。
...
## 4. 評価の時系列トラッキングがない（P1） — **解消済み**

### 現状（解消後）

`evaluate --history` が実行結果を append-only JSONL に追記する。
レポートの `delta_vs_previous` は同一 `cases_fingerprint` かつ同一 `engine` の
直近 run との精度差分。該当する過去 run がなければ `null`。
ケース `id` 欠落は警告、`--strict` でエラー。
